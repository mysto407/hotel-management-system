          ...
          {viewMode === 'detailed' && roomTypes.map(roomType => {
            ...
            return (
              <div key={roomType.id}>
                {/* Room Type Header */}
                <div
                  className="flex sticky left-0 z-20 bg-muted border-b cursor-pointer hover:bg-muted/80"
                  onClick={() => toggleRoomType(roomType.id)}
                >
                  <div
                    className="flex-shrink-0 px-1.5 py-0.5 border-r flex items-center gap-1 sticky left-0 z-20 bg-muted"
                    style={{ width: ROOM_COLUMN_WIDTH }}
                  >
                    {isCollapsed ? (
