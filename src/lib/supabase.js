...
// Sheets
export const getExpenseSheets = async(categoryId = null) => {
    let query = supabase
        .from('expense_sheets')
        .select('*')
        .order('name');
...
export const createExpenseColumn = async(sheetId, column) => {
    const { data, error } = await supabase
        .from('expense_columns')
        .insert([{
            sheet_id: sheetId,
...
export const deleteExpenseColumn = async(sheetId, columnId) => {
    const { error } = await supabase
        .from('expense_columns')
        .delete()
        .eq('sheet_id', sheetId)
...
export const createExpenseRow = async(sheetId, rowData) => {
    const { data, error } = await supabase
        .from('expense_rows')
        .insert([{
            sheet_id: sheetId,
            date: rowData.date,
            ref_no: rowData.refNo,
            total_amount: rowData.totalAmount,
            remarks: rowData.remarks,
            custom_data: rowData.customData
        }])
        .select();
    return { data, error };
};

export const updateExpenseRow = async(id, rowData) => {
    const { data, error } = await supabase
        .from('expense_rows')
        .update({
            date: rowData.date,
